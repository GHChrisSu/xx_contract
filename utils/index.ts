...
export const deployLifeDesignUpgradeableProxyAndTransferToTimelock = async (
  timeLock: Contract
) => {
  ...
  const upgrader = await upgrades.deployProxy(
    LifeDesign,
    [10, lifeDAO.address],
    {
  ...
  const lifeDesignContract = await ethers.getContractAt(
    "LifeDesign",
    lifeDesign.address
  );
  const transferTx = await lifeDesignContract.transferOwnership(
    timeLock.address
  );
